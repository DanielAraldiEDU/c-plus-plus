...
template<typename T>
int counterNodeWithChild(Node<T> *root) {
    if (root == NULL) return 0;
    if ((root->right == NULL && root->left != NULL) || (root->right != NULL && root->left == NULL)) {
        return 1 + counterNodeWithChild(root->right) + counterNodeWithChild(root->left);
    }
    return counterNodeWithChild(root->right) + counterNodeWithChild(root->left);
}
...
int main() {
    ...
    cout << "Amount nodes with child: " << counterNodeWithChild(root) << endl;
