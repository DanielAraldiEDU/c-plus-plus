...
T *handleSelectionSort(T *array, int length) {
  ...
  for (int index = 0; index < length - 1; index++)
  {
    ...
    for (auxiliaryIndex = index + 1; auxiliaryIndex < length; auxiliaryIndex++) {
        if (array[auxiliaryIndex] < array[minimumIndex]) minimumIndex = auxiliaryIndex;
    }
    if (minimumIndex != index) handleSwapArrayElement<T>(&array[minimumIndex], &array[index]);
  }
...
T *handlePopulateArray(T *array, int length) {
  for (int i = 0; i < length; i++) array[i] = createRandomNumber<T>(100);
  return array;
}

template <typename T>
void printArray(T *array, int length) {
  for (int i = 0; i < length; i++) i + 1 == length ? cout << array[i] : cout << array[i] << " ";
}
